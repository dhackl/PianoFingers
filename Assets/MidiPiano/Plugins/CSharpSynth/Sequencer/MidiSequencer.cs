...
namespace CSharpSynth.Sequencer
{
    ...
    {
		public float TempoScale { get; set; }
            ...
            if (_MidiFile.SequencerReady == false)
            {
                try
                {
                    ...
                    for (int x = 0; x < _MidiFile.Tracks[0].MidiEvents.Length; x++)
                    {
                        ...
                        {
							_MidiFile.BeatsPerMinute = (uint)(MidiHelper.MicroSecondsPerMinute / System.Convert.ToUInt32(_MidiFile.Tracks[0].MidiEvents[x].Parameters[0]) * TempoScale);
                        }
                    }
