...
namespace CSharpSynth.Banks.Fm
{
    public class FMInstrument : Instrument
    {
		...
		public override float getSampleAtTime(int note, int channel, int synthSampleRate, float tuning, ref double time)
        {
            //time
            if (time > end_time)
            {
        ...
        private void loadProgramFile(string file)
        {
			...
			StreamReader reader = new StreamReader(new FileStream(Application.dataPath + "/Resources/" + file, FileMode.Open));
